...
{
  if (card->ctx->debug) sc_debug(card->ctx, "Entering function card_check_sw\n"); 
  ...
  if (card->ctx->debug) sc_debug(card->ctx, "Leaving function card_check_sw\n"); 
...
{ 
  ...
  int old_use_virtual_fs = card_is_virtual_fs_active(card);
...
static void card_add_acl_entry(sc_card_t *card, sc_file_t *file, int op, u8 byte)
{
  ...
  if (card->ctx->debug) sc_debug(card->ctx, "Leaving function card_add_acl_entry\n"); 
}
...
static void card_parse_sec_attr(sc_card_t *card, sc_file_t *file, const u8 * buf, size_t len)
{
  ...
  if (card->ctx->debug) sc_debug(card->ctx, "Leaving function card_parse_sec_attr\n"); 
}
...
{ 
  ...
  case SC_CARDCTL_GET_SERIALNR:
    if (card->ctx->debug) sc_debug(card->ctx, "Calling function card_get_serialnr\n"); 
...
{ 
  ...
  if (card->ctx->debug) sc_debug(card->ctx, "Leaving function card_logout");
  return 0;
}
...
int card_get_serialnr(sc_card_t *card, sc_serial_number_t *serial)
{
  ...
  if (card->type != SC_CARD_TYPE_DNIE)
    return SC_ERROR_NOT_SUPPORTED;
