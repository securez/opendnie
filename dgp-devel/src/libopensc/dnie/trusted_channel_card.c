				       ...
				       u8* kicc)
{
  ...
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Entering function card_compare_signature_data\n");

  if (data_length != 128) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "data should be 128-byte long.\n");
    return SC_ERROR_INVALID_CARD;
  }
  
  if (data[0]!=0x6a || data[127]!=0xbc) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "data doesn't match 6A ... BC\n");
    return SC_ERROR_INVALID_CARD;
  }
  ...
  if (memcmp(data+127-SHA_DIGEST_LENGTH, digest, SHA_DIGEST_LENGTH) != 0) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "hashes doesn't match\n");
    return SC_ERROR_INVALID_CARD;
  }

  memcpy(kicc, data+1+74, 32);

  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "card_compare_signature_data ok!\n");
  return SC_SUCCESS;
}
				 ...
				 u8* kicc)
{
  ...
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Entering function card_verify_signature\n");

  if(signature_length != 128) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Signature should be 128-byte long\n");
    r = SC_ERROR_INVALID_CARD;
  }

  if(ifd_data_length != 0x10) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "ifd_data should be 0x10-byte long\n");
    r = SC_ERROR_INVALID_CARD;
  }
  ...
  if ((decrypted_length = RSA_private_decrypt(signature_length,
					      ...
					      ifd_private_key,				   
					      RSA_NO_PADDING)) <= 0) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Error in decryption RSA routine.\n");
  ...
  if ((encrypted_length = RSA_public_encrypt(decrypted_length,
					      ...
					      icc_public_key,				   
					      RSA_NO_PADDING)) <= 0) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Error in encryption RSA routine.\n");
    r = SC_ERROR_INVALID_CARD;
    goto dvs_end; /* to deallocate resources */
  }
  ...
  if (r != SC_SUCCESS) {
    ...
    if((min_sig_length = BN_bn2bin(min_sig_bignum, min_sig)) <= 0) {
      if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Error in converting min_sig_bignum to min_sig.\n");
      r= SC_ERROR_INVALID_CARD;
      goto dvs_end; /* to deallocate resources */
    }
 ...
 dvs_end:
  ...
  if (ctx->debug) sc_debug(ctx,SC_LOG_DEBUG_VERBOSE,  "Leaving function card_verify_signature 0x%X\n", r);
					 ...
					 )
{
  ...
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Entering function card_prepare_authenticate_data\n");
  ...
  if(!BN_sub(sub_bignum, ifd_private_key->n, decrypted_bignum)) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Error in calculating sub_bignum.\n");
    r = SC_ERROR_INVALID_CARD;
    goto dsad_end; /* to deallocate resources */
  }
  ...
  if(BN_num_bytes(min_sig_bignum) > sizeof(min_sig)) {
    if (ctx->debug) sc_debug(ctx,SC_LOG_DEBUG_NORMAL,  "Error in min_sig... it's too big.\n");
    goto dsad_end; /* to deallocate resources */
  }
 ...
 dsad_end:
  ...
  if (ctx->debug) sc_debug(ctx,SC_LOG_DEBUG_VERBOSE,  "Leaving function card_prepare_authenticate_data 0x%X\n", r);
				    ...
				    )
{
  u8 data[32+4];
  u8 digest[SHA_DIGEST_LENGTH];
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Entering function card_compute_hashed_key\n");
  ...
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Leaving function card_compute_hashed_key\n");
}
				     ...
				     u8 *ssc) /* 8-byte long buffer */
{
  ...
  if (ctx->debug) sc_debug(ctx,SC_LOG_DEBUG_VERBOSE,  "Entering function card_compute_session_keys\n");
  ...
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Leaving function card_compute_session_keys 0x%X\n", r);
  return r;
}
				  ...
				  )
{
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Entering function card_prepare_secure_tx\n");
  ...
  if(orig_apdu->lc>0) {
    ...
    if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "temp_length = 0x%X\n", temp_length);
    ...
    if(card_add_tlv(TAG_PICG, encrypted, &encrypted_length) != SC_SUCCESS) {
      if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "Error while adding tlv to encrypted data\n");
      return SC_ERROR_INTERNAL;
    }
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving function card_prepare_secure_tx\n");
  return SC_SUCCESS;
}			  
...
{
  if(buffer_length<2) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Expecting at least 2 bytes: returning SC_ERROR_INVALID_DATA\n");
  ...
  *tag = buffer[0];
  ...
  if(buffer[1]<0x80) {
    ...
    *value = buffer+4;
    ...
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Invalid length byte 0x%X\n", buffer[1]);
    return SC_ERROR_INVALID_DATA;
  }
				...
				)
{
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Entering function card_parse_secure_rx\n");
  while(p<pf) {
    if((r = card_decode_next_tlv(card->ctx, p, pf-p, &tag, &length, &value, &p)) != SC_SUCCESS) {
      if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "Error in card_decode_next_tlv. Returning 0x%X\n", r);
      return r;
    }
	...
	if((length<9) || (length>512) || ((length-1)%8)!=0) {
	  ...
	  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Invalid PIGC length\n");
	  return SC_ERROR_INVALID_DATA;
	}
	if(value[0]!=1) {
	  /* pad info byte MUST be 1 */
	  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Invalid pad info byte\n");
	  return SC_ERROR_INVALID_DATA;
	}
	/* prepare keys */
	DES_set_key_unchecked((const_DES_cblock *)(DRVDATA(card)->kenc), &k1);
	...
	if(length<8) {
	  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Invalid length. Can't have a proper padding\n");
	  return SC_ERROR_INVALID_DATA;
	}
	...
	if(ii>7) {
	  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Invalid padding (too much 0)\n");
	...
	if(temp[length] != 0x80) {
	  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Invalid padding (0x80 missing)\n");
	  return SC_ERROR_INVALID_DATA;
	}

	if(length>resplen) {
	  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Buffer too small\n");
	  return SC_ERROR_INVALID_DATA;
	}
	...
	if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Invalid TAG_SW length\n");
	return SC_ERROR_INVALID_DATA;
      }
      apdu->sw1 = value[0];
      apdu->sw2 = value[1];
    ...
    case TAG_CC:
      ...
      if(length!=4) {
	/* we expect 4 bytes*/
	if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: Invalid TAG_CC length\n");
	return SC_ERROR_INVALID_DATA;
      }
	...
	if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_NORMAL,  "ERROR: Too much data to calculate mac\n");
	return SC_ERROR_INVALID_DATA;
      }
	...
	if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_NORMAL,  "ERROR: macs don't match\n");
	return SC_ERROR_INVALID_DATA;
      }
  ...
  if((tags_checked&0x6) != 0x6) {
    ...
    if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "ERROR: missing TAG_SW or TAG_CC\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving function card_parse_secure_rx\n");
  return SC_SUCCESS;
}
...
int card_card_create_secure_channel(struct sc_card *card)
{
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Entering Step H\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving Step H\n");

  /* Stage 1 */

  /* Step A: */
  /* Step B: Select CA Root public key on smart card*/
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Entering Step B\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving Step B\n");

  /* Step C: Verify the certificate of a CA's public key C_CV.CA.CS_AUT */
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Entering Step C\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving Step C\n");

  /* Step D: Select key verification*/
  
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Entering Step D\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving Step D\n");

  /* Step E: Verify CA Certificate*/

  if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_VERBOSE,  "Entering Step E\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_VERBOSE,  "Leaving Step E\n");
  
  /* Stage 3 */

  /* Step I: Select two keys for verification */

  if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_VERBOSE,  "Entering Step I\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving Step I\n");

  /* Step J: Internal Authenticate*/

  if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_VERBOSE,  "Entering Step J\n");
  ...
  if((r = card_verify_signature(card->ctx,
				...
				icc_public_key,
				kicc) ) != SC_SUCCESS) {
    if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "card_verify_signature failed");
    goto dccsc_end;
  }
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Entering Step K\n");
  ...
  if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_VERBOSE,  "Entering Step L\n");
  ...
  if ((r = card_sign_authentication_data(card->ctx,
    ...
    if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "card_sign_authentication_data failed");
    goto dccsc_end;
  }
  ...
  if((r=card_compute_session_keys(card->ctx, 
    ...
    if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_NORMAL, "card_compute_session_keys failed!");
    goto dccsc_end;
  }

  if (card->ctx->debug) sc_debug(card->ctx,SC_LOG_DEBUG_VERBOSE,  "Leaving Step L\n");
 ...
 dccsc_end:
  ...
  if (card->ctx->debug) sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Leaving function card_card_create_secure_channel with errorcode 0x%X\n", r);
  return r;
}
