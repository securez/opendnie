				     ...
				     u8 **buf_out, size_t *bufsize_out)
{
  ...
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Entering function sc_standard_pkcs15_encode_df\n");
  ...
  if (func == NULL && func2 == NULL) {
    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Going to encode the other PKCS#15 DF\n");
  ...
  if (func2){
    ...
    tokeninfo.serial_number = p15card->tokeninfo->serial_number;
    tokeninfo.manufacturer_id = p15card->tokeninfo->manufacturer_id;
    tokeninfo.last_update = p15card->tokeninfo->last_update;
    tokeninfo.preferred_language = p15card->tokeninfo->preferred_language;

    if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Going to encode TokenInfo PKCS#15 DF\n");
    r = sc_pkcs15_encode_tokeninfo( ctx, &tokeninfo, buf_out, bufsize_out );
    goto sspead_out;

  }
  /* decode ODF or UNUSED_SPACE PKCS#15 DF */
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Going to encode ODF or UNUSED_SPACE PKCS#15 DF\n");
  r = func( ctx, p15card, buf_out, bufsize_out );
  
 sspead_out:
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Leaving function sc_standard_pkcs15_encode_df\n");
  return r;
}
				       ...
				       u8 **buf_out, size_t *bufsize_out)
{
  ...
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Entering function sc_standard_pkcs15_encode_other_df\n");
 ...
 sspeod_out:
  if (ctx->debug) sc_debug(ctx, SC_LOG_DEBUG_VERBOSE, "Leaving function sc_standard_pkcs15_encode_other_df\n");
  return r;
}
