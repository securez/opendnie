...
int card_sync_card_to_virtual_fs_filter_prkey( sc_card_t *card, struct _virtual_file_t *virtual_file, virtual_fs_t *virtual_fs, sc_pkcs15_object_t *obj )
{
  ...
  if(prkey) {
    /* set asn1 to map */
    r = map_id_to_der_set_item(DRVDATA(card)->prkdf_card_ckaid_to_card_der_map, &prkey->id, &obj->content);
    if(r != SC_SUCCESS)
      goto end;
    ...
    sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE,"Pointer to prkey info was empty");
  }
